...
thread_local char **labels; // 线程局部的动态 labels 数组指针
...
static int loadLabelName(const char *locationFilename, char *label[], int obj_class_num)
{
    printf("load lable %s\n", locationFilename);
    int res = readLines(locationFilename, label, obj_class_num);
#if 0
    if(res > 0){
        printf("loadLabelName %s successful!\n", locationFilename);
        for(int i = 0; i < res; i++){
            printf("%s\n", label[i]);
        }
    }
    else {
        printf("loadLabelName %s failed!\n", locationFilename);
    }
#endif
    return 0;
}
...
static int process_u8(uint8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                      ...
                      int32_t zp, float scale, int obj_class_num, int prop_box_size)
{
    int validCount = 0;
...
static int process_fp32(float *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold, int obj_class_num, int prop_box_size)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold,
                 float nms_threshold, object_detect_result_list *od_results, int obj_class_num, int prop_box_size)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant)
        {
            validCount += process_i8_rv1106((int8_t *)(_outputs[i]->virt_addr), (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
                                            classId, conf_threshold, app_ctx->output_attrs[i].zp, app_ctx->output_attrs[i].scale, obj_class_num, prop_box_size);
        }
#elif defined(RKNPU1)
        // NCHW reversed: WHCN
        grid_h = app_ctx->output_attrs[i].dims[1];
        ...
        else
        {
            validCount += process_fp32((float *)_outputs[i].buf, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
                                       classId, conf_threshold, obj_class_num, prop_box_size);
        }
#else
...
int init_post_process(int obj_class_num, const char *label_text_path)
{
    labels = new char *[obj_class_num]; // 动态分配线程独立的 labels 数组
    int ret = 0;
    ret = loadLabelName(label_text_path, labels, obj_class_num);
    if (ret < 0)
    {
...
char *coco_cls_to_name(int obj_class_num, int cls_id)
{
...
void deinit_post_process(int obj_class_num)
{
